...
const FROM_EMAIL = 'DEEPDELL <hello@support.deepdell.com>';
...
export async function onRequestPost({request,env}) {
  ...
  const name=String(data.name||'').trim(),email=String(data.email||'').trim(),company=String(data.company||'').trim();
  const website=String(data.website||'').trim(),service=String(data.service||'').trim(),phone=String(data.phone||'').trim();
  const message=String(data.message||'').trim(),auditUrl=String(data.audit_url||'').trim();
  ...
  const subject=auditUrl?`New Agentic Website Lead — ${auditUrl.replace(/^https?:\/\//i,'').replace(/\/$/,'')}`:`New DEEPDELL Project Enquiry — ${service}`;
  ...
  const response=await fetch('https://api.resend.com/emails',{method:'POST',headers:{'Authorization':`Bearer ${env.RESEND_API_KEY}`,'Content-Type':'application/json'},body:JSON.stringify({from:FROM_EMAIL,to:[TO_EMAIL],reply_to:email,subject,html})});
  if(!response.ok){console.error('Resend error:',await response.text());return json({error:'The email could not be sent. Please try again or email support@deepdell.com.'},502)}
  return json({ok:true});
}